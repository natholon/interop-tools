...
async def data_specification_form(
    ...
    context.update({"hl7_text": raw_text, "crosswalk_result": outcome.report_json, "crosswalk_error": error})
...
async def data_specification_api(payload: CrosswalkApiRequest):
    ...
    return JSONResponse(content={"bundle": json.loads(outcome.bundle_json), "report": json.loads(outcome.report_json)})
