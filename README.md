...
**Planned next:** the Data Specification pillar has reached full breadth across every format and every section this app's conversion recognizes - HL7v2 (all four message types), X12 EDI (the complete "big five" suite), and C-CDA (every general-purpose section, every narrative-only section, and the real structured entries Plan of Treatment/Social History/Family History can carry beneath their own narrative text). What remains before a C-CDA document type could be marked fully supported: a CDA-side Provenance resource builder (so `author` produces a fact for any section that reads one), Procedures' own Indication/Comment Activity cross-references (not yet parsed into any FHIR field), and resolving a structured section's coded field back to its own narrative text when a display name is absent (currently falls back to code-only) - each a bounded follow-up on existing infrastructure, not a redesign.
