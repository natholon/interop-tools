...
def test_index_transform_target_dropdown_includes_835():
    response = client.get("/")
    assert response.status_code == 200
    assert "EDI 835" in response.text


def test_api_transform_builds_835_interchange():
    convert_response = client.post("/api/convert", json={"hl7_text": read_fixture("edi_835_basic.x12")})
    bundle_json = json.dumps(convert_response.json()["bundle"])

    response = client.post(
        "/api/transform",
        json={"bundle_json": bundle_json, "target_format": "EDI", "target_type": "835", "target_trigger": ""},
    )
    assert response.status_code == 200
    message_text = response.json()["message_text"]
    assert message_text.startswith("ISA*")
...
def test_index_transform_target_dropdown_includes_837p():
    response = client.get("/")
    assert response.status_code == 200
    assert "EDI 837P" in response.text


def test_api_transform_builds_837p_interchange():
    convert_response = client.post("/api/convert", json={"hl7_text": read_fixture("edi_837p_basic.x12")})
    bundle_json = json.dumps(convert_response.json()["bundle"])

    response = client.post(
        "/api/transform",
        json={"bundle_json": bundle_json, "target_format": "EDI", "target_type": "837P", "target_trigger": ""},
    )
    assert response.status_code == 200
    message_text = response.json()["message_text"]
    assert message_text.startswith("ISA*")
...
def test_index_transform_target_dropdown_includes_837d():
    response = client.get("/")
    assert response.status_code == 200
    assert "EDI 837D" in response.text


def test_api_transform_builds_837d_interchange():
    convert_response = client.post("/api/convert", json={"hl7_text": read_fixture("edi_837d_basic.x12")})
    bundle_json = json.dumps(convert_response.json()["bundle"])

    response = client.post(
        "/api/transform",
        json={"bundle_json": bundle_json, "target_format": "EDI", "target_type": "837D", "target_trigger": ""},
    )
    assert response.status_code == 200
    message_text = response.json()["message_text"]
    assert message_text.startswith("ISA*")
